...
multiTabsBtns.forEach((tabBtn, index) => {
  ...
  if (!btn.disabled) {
    ...
    btn.addEventListener("click", () => {
      // Remove "active_tab" from all buttons
      multiTabsBtns.forEach((_tabBtn) =>
        _tabBtn.classList.remove("active_tab")
      );

      // Add "active_tab" to the clicked button
      tabBtn.classList.add("active_tab");

      // Remove "active" from all tabs
      multiTabs.forEach((tab) => tab.classList.remove("active"));

      // Add "active" to the corresponding tab
      multiTabs[index].classList.add("active");

      // Dynamically enable the next step button
      if (index < multiTabsBtns.length - 1) {
        const nextBtn = multiTabsBtns[index + 1].querySelector(".tab-btn");
